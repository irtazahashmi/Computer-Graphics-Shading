...
void showImGuizmoTranslation(const Window& window, const Trackball& camera, glm::vec3& position)
{
    ...
    if (manipulated) {
        glm::vec3 dummyRotate, dummyScale; // Unused; DecomposeMatrixToComponents does not support passing nullptr.
        ImGuizmo::DecomposeMatrixToComponents(
            glm::value_ptr(modelMatrix), glm::value_ptr(position), glm::value_ptr(dummyRotate), glm::value_ptr(dummyScale));
    }
}
