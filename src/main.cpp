...
glm::vec3 computeLighting(const ProgramState& programState, unsigned vertexIndex, const glm::vec3& cameraPos, const Light& light)
{
    ...
    if (programState.displayModes.debug) {
        return debugColor(programState.shadingData, vertex.position, vertex.normal, light.position, cameraPos);
    }

    glm::vec3 result { 0.0f };
    switch (programState.displayModes.diffuse) {
    case DiffuseMode::Lambert: {
        result += diffuseOnly(programState.shadingData, vertex.position, vertex.normal, light.position);
    } break;
    case DiffuseMode::ToonDiffuse: {
        result = toonShadingNoSpecular(programState.shadingData, vertex.position, vertex.normal, light.position);
    } break;
    ...
    case SpecularMode::Phong: {
        result += phongSpecularOnly(programState.shadingData, vertex.position, vertex.normal, light.position, cameraPos);
    } break;
    case SpecularMode::BlinnPhong: {
        result += blinnPhongSpecularOnly(programState.shadingData, vertex.position, vertex.normal, light.position, cameraPos);
    } break;
    case SpecularMode::ToonSpecular: {
        result += toonShadingOnlySpecular(programState.shadingData, vertex.position, vertex.normal, light.position, cameraPos);
    } break;
...
void drawUI(ProgramState& state, const Trackball& camera)
{
    ...
    ImGui::SliderFloat("Shininess", &state.shadingData.shininess, 1.0f, 30.0f, "%.1f", ImGuiSliderFlags_Logarithmic);
    ImGui::SliderInt("Toon discrete steps", &state.shadingData.toonDiscretize, 2, 20);
...
int main(int argc, char** argv)
{
    ...
    ProgramState state {};
    state.myMesh = loadMesh(argv[1] ? argv[1] : std::filesystem::path(DATA_DIR) / "DavidHeadCleanMax.obj", true)[0];
